...
TEAM_MODEL_DIR = BASE_DIR / "ensemble_models"
TEAM_FEATURES = [
...
def load_team_hmm(df_game):
    try:
        ...
        for a in ["SPY", "TLT", "SHV", "GLD", "DBC"]:
            if f"{a}_ret" not in df.columns:
                df[f"{a}_ret"] = df[a].pct_change().fillna(0)
        scaler = StandardScaler()
        X = scaler.fit_transform(
            df[TEAM_FEATURES].replace([float('inf'), float('-inf')], float('nan')).ffill().bfill()
        )
        hmm = GaussianHMM(n_components=HMM_N_STATES, covariance_type="full",
                          n_iter=1000, random_state=42)
        hmm.fit(X)
        states_raw = hmm.predict(X)
        vix_mean   = {s: df["VIX"].values[states_raw == s].mean() for s in range(HMM_N_STATES)}
        ...
        return hmm, scaler, probs6, X_scaled, df
    except Exception:
...
def rolling_sharpe_score(values):
    ...
    if len(arr) < 3:
        return 0.0
    return float(arr.mean() / (arr.std() + 1e-8) * np.sqrt(252))
...
def ai_panel_weights(row, assets, level, df_full=None):
    model_ppo, model_a2c = load_team_models()
    if model_ppo is None or df_full is None:
        return _rule_based_weights(row, assets, level)
    try:
        hmm, scaler, probs6, X_scaled, df_feat = load_team_hmm(df_full)
        if hmm is None:
            return _rule_based_weights(row, assets, level)
        ...
        recent_rets = df_feat[ret_cols].values[target_idx-lookback:target_idx]
        ppo_rets, a2c_rets = [], []
        ...
        ppo_ratio = ratio_from_scores(rolling_sharpe_score(ppo_rets), rolling_sharpe_score(a2c_rets))
...
def gemini_expert_review(actual_level, guess, user_w, ai_w, user_return, ai_return, row):
    """전문가 요약만 생성 (PDF 저장용)"""
    if gemini is None:
    ...
    guessed  = REGIMES[guess]["finance"]
    if guess == actual_level:
        guess_comment = f"국면 예측 정확 ({regime['finance']})"
    else:
        guess_comment = f"예측: {guessed} → 실제: {regime['finance']}"
    if user_return > ai_return:
        result_str = f"인간 패널 승리 ({user_return:.2%} vs AI {ai_return:.2%})"
    elif ai_return > user_return:
        result_str = f"AI 패널 P 승리 ({ai_return:.2%} vs 인간 {user_return:.2%})"
    else:
        result_str = f"무승부 ({user_return:.2%})"

    prompt = f"""
당신은 금융 전문가입니다. 아래 데이터를 바탕으로 전문적이고 명확한 투자 분석 요약을 작성해주세요.
...
[AI 패널 P 자산 배분]
SPY {ai_w['SPY']:.1%} / TLT {ai_w['TLT']:.1%} / SHV {ai_w['SHV']:.1%} / GLD {ai_w['GLD']:.1%} / DBC {ai_w['DBC']:.1%}

[인간 패널 자산 배분]
SPY {user_w['SPY']:.1%} / TLT {user_w['TLT']:.1%} / SHV {user_w['SHV']:.1%} / GLD {user_w['GLD']:.1%} / DBC {user_w['DBC']:.1%}

[결과] {result_str}
[국면 예측] {guess_comment}

아래 형식으로 전문가 요약을 작성해주세요 (이모지 없이, 명확하고 전문적인 문체로):

1. 시장 상황 분석
(VIX, 금리차, SPY 모멘텀 등 주요 지표를 바탕으로 당시 시장 상황 2~3문장)

2. AI 자산 배분 근거
(AI가 이렇게 배분한 경제적 이유 3~4문장)

3. 대결 결과 분석
(수익률 차이가 난 원인과 시사점 2~3문장)

4. 투자 인사이트
(이 국면에서 얻을 수 있는 핵심 투자 교훈 1~2문장)
"""
    try:
        response = gemini.generate_content(prompt)
        return response.text
    except Exception as e:
        return f"(전문가 요약 생성 실패: {e})"
...
if st.session_state.submitted and st.session_state.user_weights is not None:
    ...
    # ── 전문가 요약 PDF 저장 ──────────────────────────────────────────────────
    st.markdown("---")
    if st.button("📊 전문가 요약 PDF 저장하기", use_container_width=False):
        with st.spinner("전문가 요약 생성 중..."):
            expert_text = gemini_expert_review(
                actual_level, guess,
                user_w, ai_w,
                user_m["total_return"], ai_m["total_return"],
                row
            )
        # PDF 생성 (reportlab 없이 텍스트 기반 PDF)
        try:
            ...
            title_style = ParagraphStyle("title", fontName=font_name,
                                         fontSize=16, spaceAfter=8, leading=20)
            body_style  = ParagraphStyle("body",  fontName=font_name,
                                         fontSize=11, spaceAfter=6, leading=16)
            meta_style  = ParagraphStyle("meta",  fontName=font_name,
                                         fontSize=9,  spaceAfter=4, leading=13,
                                         textColor="#666666")

            story = []
            story.append(Paragraph("마켓시그널 — 전문가 투자 분석 요약", title_style))
            story.append(Spacer(1, 4*mm))
            story.append(Paragraph(f"회차: {REGIMES[actual_level]['episode']}", meta_style))
            story.append(Paragraph(f"관찰 날짜: {date.strftime('%Y-%m-%d')}", meta_style))
            story.append(Paragraph(
                f"플레이어 수익률: {user_m['total_return']:.2%} / AI 수익률: {ai_m['total_return']:.2%}",
                meta_style))
            story.append(Spacer(1, 4*mm))
            ...
            st.download_button(
                label="⬇️ PDF 다운로드",
                data=buf,
                file_name=f"marketsignal_{date.strftime('%Y%m%d')}.pdf",
                mime="application/pdf",
            )
        except ImportError:
            # reportlab 없으면 텍스트 파일로 fallback
            txt = f"마켓시그널 전문가 투자 분석 요약\n"
            txt += f"회차: {REGIMES[actual_level]['episode']}\n"
            txt += f"날짜: {date.strftime('%Y-%m-%d')}\n"
            txt += f"플레이어: {user_m['total_return']:.2%} / AI: {ai_m['total_return']:.2%}\n\n"
            txt += expert_text
            st.download_button(
                label="⬇️ TXT 다운로드 (reportlab 미설치)",
                data=txt.encode("utf-8"),
                file_name=f"marketsignal_{date.strftime('%Y%m%d')}.txt",
                mime="text/plain",
            )
